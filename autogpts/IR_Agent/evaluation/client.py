...
async def main():
    ...
    async with ApiClient(configuration) as api_client:
        ...
        task_request_body = TaskRequestBody(input="What is a qersu?")
        ...
        i = 1
        step_request_body = StepRequestBody(input=task_request_body.input)
        while (
            step := await api_instance.execute_agent_task_step(
                task_id=task_id, step_request_body=step_request_body
            )
        ) and step.is_last is False:
            print("The response of AgentApi->execute_agent_task_step:\n")
            print(step)
            print("\n\n")
            step_request_body = StepRequestBody()
            i += 1
